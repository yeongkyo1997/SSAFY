...
public class UserDaoImpl implements UserDao {
    ...
    public User selectLogin(User user) throws SQLException {
        try (Connection con = dataSource.getConnection(); PreparedStatement pstmt = con.prepareStatement("  select userid, userpw, position, name, rname, rclass " + "  from userinfo " + " where userid = ? and userpw = ?");) {
